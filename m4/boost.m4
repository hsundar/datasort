...
# if BOOST_DIR is set and BOOST_ROOT is not, use it.
...
#----------------------------------------------------------------------
#----------------------------------------------------------------------
# koomie mod (1/2/2012) - swap boost_mt order to be more in line with
# the intended algorithm search order described above; avoids problem
# of finding mt-versions of a boost lib in a system path which are not
# provided in the user-provided directory given by --with-boost.  You
# can always include "mt" in the PREFERRED-RT-OPT if you want to give
# preferential treatment to mt versions.

dnl original for boost_mt_ in $boost_mt -mt ''; do
...
dnl PB: -R option breaks gcc. It looks like Intel doesn't use it either...
dnl     line was originally: Boost_lib_LDFLAGS="-L$boost_ldpath -R$boost_ldpath"
