...
namespace ot {
  ...
  unsigned int my_wt_fn(const TreeNode* n){
    return n->getLevel();
  }
  ...
  int balanceOctree (std::vector<TreeNode > &in, std::vector<TreeNode > &out,
    ...
    DendroIntL locInSize = in.size();
    out.clear();

    //////////////////////////////////////////////////////////////////////////////////////////////////

    //Redistribute.
    par::partitionW<TreeNode>(in, my_wt_fn , comm);

    //Build level-by-level set of nodes.
    std::vector<std::set<TreeNode> > nodes(maxDepth+1);
    for(size_t i=0;i<in.size();i++)
      nodes[in[i].getLevel()].insert(in[i]);

    //Add new nodes level-by-level.
    unsigned int s=1;
    unsigned int num_chld=1u<<dim;
    unsigned int maxCoord=(1u<<maxDepth);
    unsigned int num_nbrs=(unsigned int)(pow(3,dim)+0.5);
    for(unsigned int l=maxDepth;l>1;l--){
      //Build set of parents of balancing nodes.
      std::set<TreeNode> nbrs_parent;
      for(std::set<TreeNode>::iterator node=nodes[l].begin(); node != nodes[l].end();){
        unsigned int pX=node->getParentX();
        unsigned int pY=node->getParentY();
        unsigned int pZ=node->getParentZ();
        for(int i=0; i<num_nbrs; i++){
          int dX = ((i/1)%3-1)*s*2;
          int dY = ((i/3)%3-1)*s*2;
          int dZ = ((i/9)%3-1)*s*2;
          unsigned int newX=pX+dX;
          unsigned int newY=pY+dY;
          unsigned int newZ=pZ+dZ;
          if((int)newX-(int)pX==dX && newX+s<=maxCoord)
          if((int)newY-(int)pY==dY && newY+s<=maxCoord)
          if((int)newZ-(int)pZ==dZ && newZ+s<=maxCoord){
            TreeNode nbr(newX,newY,newZ,l-1,dim,maxDepth);
            nbrs_parent.insert(nbr.getParent());
          }
        }
        TreeNode xnode=*node;
        while(xnode==*node) node++;
      }
      //Get the balancing nodes.
      for(std::set<TreeNode>::iterator node=nbrs_parent.begin(); node != nbrs_parent.end(); node++){
        std::vector<TreeNode> children;
        node->addChildren(children);
        for(int j=0;j<num_chld;j++)
          nodes[l-1].insert(children[j]);
      }
      s*=2;
    }
    ...
    for(unsigned int l=1;l<=maxDepth;l++)
      for(std::set<TreeNode>::iterator node=nodes[l].begin(); node != nodes[l].end(); node++){
        TreeNode parent=node->getParent();
        nodes[l-1].erase(parent);
      }
    ...
    DendroIntL locTmpSize = in.size();

    //Sort, Linearise, Redistribute.
    MPI_Barrier(comm);
    double tt=omp_get_wtime();
    par::sampleSort(in, out, comm);
    MPI_Barrier(comm);
    if(!rank) std::cout<<omp_get_wtime()-tt<<'\n';

    lineariseList(out, comm);
    par::partitionW<TreeNode>(out, NULL , comm);

    //////////////////////////////////////////////////////////////////////////////////////////////////

#ifndef __SILENT_MODE__
    DendroIntL locOutSize = out.size();
    DendroIntL globInSize, globTmpSize, globOutSize;
    par::Mpi_Allreduce<DendroIntL>(&locInSize , &globInSize , 1, MPI_SUM, comm);
    par::Mpi_Allreduce<DendroIntL>(&locTmpSize, &globTmpSize, 1, MPI_SUM, comm);
    par::Mpi_Allreduce<DendroIntL>(&locOutSize, &globOutSize, 1, MPI_SUM, comm);
  ...
  //Assumption: in is globally sorted, linear and complete.
  int balanceOctree1 (std::vector<TreeNode > &in, std::vector<TreeNode > &out,
      unsigned int dim, unsigned int maxDepth, bool incCorner,
      MPI_Comm comm, MPI_Comm* newCommPtr, bool* iAmActive) {
#ifdef __PROF_WITH_BARRIER__
    MPI_Barrier(comm);
